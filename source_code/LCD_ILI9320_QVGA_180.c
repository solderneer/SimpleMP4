...
void LCD_Init(unsigned int type)
{
	LCD_Reg_Set(0x01, 0x0100);
	LCD_Reg_Set(0x03, 0x10B8);
	LCD_Reg_Set(0x60, 0xA700);	
...
void  PowerOnLcdinit()
{
	...
	Delay1Ms(50);
	//LCD_Reg_Set(0x13, 0x1600); //0x1300); //0x1800);                // VDV[4:0] for VCOM amplitude
	//LCD_Reg_Set(0x29, 0x0012); //0x0004); //0x0005);                // VCM[4:0] for VCOMH
	
	LCD_Reg_Set(0x13, 0x1300); //0x1300); //0x1800);
	LCD_Reg_Set(0x29, 0x0004); //0x0004); //0x0005);                // VCM[4:0] for VCOMH
	...
	// ---------- Gate Scan Control  ---------- //
	LCD_Reg_Set(0x60, 0xA700);		// GS, NL[5:0], SCN[5:0]
...
{
	LCD_Reg_Set(0x01, 0x0000);
	LCD_Reg_Set(0x03, 0x10B0);
	LCD_Reg_Set(0x60, 0xA700);
    LCDDEV_SetWindow(0, 0,  LCD_MAX_XSIZE-1, LCD_MAX_YSIZE-1); 	
}
