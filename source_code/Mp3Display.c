...
unsigned int Mp3PlayEqStr[12][8] __attribute__((section(".mp3_data, \"aw\""))) =
{
    {"  NOR  "},
    {"  3D   "},
    {" ROCK  "},
    ...
    while (TempDispFlag != 0) {
        ...
        if (TempDisplayFlag1 != 0) {
            ...
            case MUSIC_DISPFLAG_SPECTRUM:
            {
                for(i = 0; i < 17; i ++) {
                    DisplayPicture(MUSIC_SPECTRUM_PIC_X + (i * (MUSIC_SPECTRUM_PIC_XSIZE + 0)), MUSIC_SPECTRUM_PIC_Y, 
                                   ...
                                   (NFA_MUSIC_SPECTRUM_PIC + (unsigned long)(StroboscopeArray[i]) * MUSIC_SPECTRUM_PIC_SIZE));
                }
                break;
            }
            ...
            case MUSIC_DISPFLAG_CUR_TIME:
            {
                ...
                GetTimeHMS(Mp3CurrTimeSec,&i,&j,&k);
                ...
                tempL = 0;
                if (Mp3TotalTimeSec >= Mp3CurrTimeSec) tempL = Mp3TotalTimeSec - Mp3CurrTimeSec;
            ...
            case MUSIC_DISPFLAG_LRC:
            {
                ...
                LCD_DispStringInRect(&r, &r, LrcBuffer, LCD_TEXTALIGN_LEFT);
                LCD_SetColor(TempColor);
                break;
            }
            ...
            case MUSIC_DISPFLAG_STATUS:
            {
                DisplayPicture(MUSIC_PLAY_STATUS_PIC_X, MUSIC_PLAY_STATUS_PIC_Y, MUSIC_PLAY_STATUS_PIC_XSIZE, MUSIC_PLAY_STATUS_PIC_YSIZE, 
                              (MUSIC_STATUS_PLAY_PIC + (unsigned long)(Mp3PlayStatus) * MUSIC_PLAY_STATUS_PIC_SIZE));
                break;
            }
...
//                } else {
//                    LCD_ScrollStringAt(&r, Id3Info.id3_title, MUSIC_ID3BK_TITLE_PIC_X, MUSIC_ID3BK_PIC_XSIZE, MUSIC_ID3BK_TITLE_PIC);
//                }
                
                r.y0 = MUSIC_ID3ART_TXT_Y;
                r.y1 = MUSIC_ID3ART_TXT_Y + CH_CHAR_YSIZE - 1;
                LCD_DispStringInRect(&r,&r,Id3Info.id3_singer, LCD_TEXTALIGN_LEFT);
                
                r.y0 = MUSIC_ID3ALBUM_TXT_Y;
                r.y1 = MUSIC_ID3ALBUM_TXT_Y + CH_CHAR_YSIZE - 1;
                LCD_DispStringInRect(&r,&r,Id3Info.id3_album, LCD_TEXTALIGN_LEFT);
                LCD_SetColor(TempColor);
                break;
            }
            ...
            case MUSIC_DISPFLAG_EQ:
            {
                ...
                TempColor = LCD_GetColor();
                LCD_SetColor(COLOR_BLACK);
                LCD_DispStringInRect(&r, &r, &Mp3PlayEqStr[MusicPlayInfo.Eq][0], LCD_TEXTALIGN_CENTER);
                LCD_SetColor(TempColor);
                break;
            }
